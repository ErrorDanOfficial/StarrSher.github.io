...
export function createGame({ canvas, storage, audio, ui }){
  ...
  const state = {
    ...
    levelMode: { active:false, level:0, wave:0, wavesTarget:0, tower:false },
  ...
  function update(dt){
    ...
    state.timeLeft -= dt; if (state.timeLeft <= 0) { endRound();
      if (state.levelMode.active){
        state.levelMode.wave++;
        ui.updateWavebar(state.levelMode.wave);
        if (state.levelMode.wave >= state.levelMode.wavesTarget){
          completeLevel();
        } else { startRound(); }
      } else {
        state.round++; startRound(); ui.updateRound(state.round);
      }
    }
  ...
  function gameOver(){
    ...
    // If in level mode and died, do not open next level; allow retry which resets level progress
    if (state.levelMode.active){ state.levelMode.wave = 0; }
  }

  function completeLevel(){
    state.running=false;
    // open next level
    if (!state.levelMode.tower){ const maxLevel = storage.get('max_level',1); if (state.levelMode.level >= maxLevel) storage.set('max_level', state.levelMode.level+1); }
    const gainedXP = Math.floor(state.score / 100); state.totalXP += gainedXP; storage.set('xp', state.totalXP);
    ui.showLevelWin({ score: state.score, roundXP: gainedXP, totalXP: state.totalXP, nextAvailable: true });
  }
  ...
  const api = {
    ...
    getState: ()=> state,
    startLevel(level){ state.levelMode.active=true; state.levelMode.level=level; state.levelMode.tower=false; state.levelMode.wave=0; state.levelMode.wavesTarget = wavesForLevel(level); state.round=1; state.lives=3; state.score=0; state.kills=0; state.enemySpawnCount=3; state.echoes=[]; startRound(); requestAnimationFrame(tick); },
    startTower(){ state.levelMode.active=true; state.levelMode.tower=true; state.levelMode.level=1; state.levelMode.wave=0; state.levelMode.wavesTarget = wavesForTower(1); state.round=1; state.lives=3; state.score=0; state.kills=0; state.enemySpawnCount=3; state.echoes=[]; startRound(); requestAnimationFrame(tick); },
    getWaveTarget(){ return state.levelMode.wavesTarget || 0; }
  };
...
function wavesForLevel(l){ if (l<4) return 3; if (l<10) return 5; if (l<15) return 10; if (l<20) return 13; if (l<28) return 16; if (l<30) return 18; return 20; }
function wavesForTower(n){ if (n<10) return 10; if (n<20) return 20; if (n<50) return 30; return 35; }
